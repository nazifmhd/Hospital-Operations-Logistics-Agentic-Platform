...
_equipment_agent = None

def get_equipment_agent():
    """Get or create equipment tracker agent instance"""
    global _equipment_agent
    if _equipment_agent is None:
        _equipment_agent = EquipmentTrackerAgent()
...
async def predict_maintenance_needs(
    ...
    try:
        ...
        predictions = await get_equipment_agent().predict_maintenance_needs([
            {
                "id": eq.id,
                "name": eq.name,
...
async def optimize_equipment_allocation(
    ...
    try:
        ...
        optimization = await get_equipment_agent().optimize_equipment_allocation([
            {
                "id": eq.id,
                "name": eq.name,
                "equipment_type": eq.equipment_type,
