...
from ...models.supply_models import Supply, SupplyCreate, SupplyUpdate
from ...agents.supply_inventory_agent import SupplyInventoryAgent

router = APIRouter()

# Initialize the supply inventory agent
...
@router.get("/low-stock/", response_model=List[Supply])
...
@router.get("/expired/", response_model=List[Supply])
...
@router.get("/expiring-soon/", response_model=List[Supply])
...
@router.get("/category/{category}/", response_model=List[Supply])
...
async def get_inventory_analytics(
    ...
    try:
        ...
        analytics = await supply_agent.get_inventory_analytics(category=category)
        return analytics
...
async def predict_supply_demand(
    ...
    try:
        ...
        predictions = await supply_agent.predict_supply_demand([
            {
                "id": s.id,
                "name": s.name,
                "category": s.category,
...
async def optimize_procurement(
    ...
    try:
        ...
        optimization = await supply_agent.optimize_supply_procurement([
            {
                "id": s.id,
                "name": s.name,
                "category": s.category,
