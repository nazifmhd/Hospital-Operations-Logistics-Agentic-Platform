...
from ...models.bed_models import Bed, BedCreate, BedUpdate
from ...agents.bed_management_agent import BedManagementAgent

router = APIRouter()

# Initialize the bed management agent
...
@router.get("/available/", response_model=List[Bed])
...
@router.get("/department/{department}/", response_model=List[Bed])
...
async def predict_bed_demand(
    ...
    try:
        ...
        demand_prediction = await bed_agent.predict_bed_demand(
...
async def optimize_bed_allocation(
    ...
    try:
        ...
        optimization = await bed_agent.optimize_bed_allocation([
            {
                "id": bed.id,
